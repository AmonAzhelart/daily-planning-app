...
import {
  ...
  AddCircleOutline as AddCircleOutlineIcon, // Per il bottone aggiungi intervento
...
const CollapsibleTableRow: React.FC<CollapsibleTableRowProps> = ({
  ...
  const handleAddInterventionFromSearch = () => {
    ...
    setInterventionSearchTerm(null); // Resetta il termine di ricerca dopo l'aggiunta
    setInterventionInputValue(''); // Resetta l'input text dell'autocomplete
  };
  ...
  // Rendering per la modalità modale mobile (Migliorato UX/UI - Senza Grid)
        ...
        <Stack spacing={3}>
          ...
          <Paper elevation={0} sx={{ p: 2, border: `1px solid ${theme.palette.divider}`, borderRadius: 1.5 }}>
            ...
            <Stack spacing={2}>
              ...
              />
              <Stack direction="row" spacing={2} alignItems="flex-start"> {/* Changed to flex-start for label alignment */}
                <Box sx={{ width: '50%' }}>
                ...
                <Box sx={{ width: '50%' }}>
                  ...
                  <Tooltip title={rowData.materialAvailable ? "Materiale disponibile" : "Materiale da verificare"}>
                    <Chip
                      ...
                      variant="filled"
                      sx={{ color: 'common.white', fontWeight: 500, width: '100%', height: 40,
  ...
  // Rendering Desktop esistente (riga di riepilogo + riga collassabile)
  return (
    <React.Fragment>
      <TableRow
        hover
        ...
        <TableCell style={{ padding:0, borderBottom: open ? `3px solid ${theme.palette.primary.main}` : 'none' }} colSpan={userRole === 'SPECIALIST' ? 7 : 6}>
          <Collapse in={open} timeout={300} unmountOnExit>
            <Box sx={{ p: {xs: 2, md: 2.5}, backgroundColor: alpha(theme.palette.primary.main, 0.02) }}>
              ...
              <Stack spacing={2.5}>
                {userRole === 'MAGAZZINIERE' && (
                    <TextField label="Descrizione Evento (Zoho)" value={rowData.zohoSuggestion} InputProps={{ readOnly: true, startAdornment:(<InputAdornment position="start"><InfoOutlinedIcon color="action"/></InputAdornment>) }} variant="filled" multiline fullWidth helperText="Campo informativo da Zoho Calendar." />
                )}
                {userRole === 'SPECIALIST' && (
                     <Autocomplete
                        options={allRisorse}
