...
export const useDailyPlanningApi = () => {
    ...
    const getClients = useCallback(() => wrappedApiCall<ClienteResponseAPI[]>('/views/clients/', 'GET'), [wrappedApiCall]);
    const getInterventionTypes = useCallback(() => wrappedApiCall<TipoIntervento[]>('/views/intervention_types/', 'GET'), [wrappedApiCall]);
    const getResources = useCallback(() => wrappedApiCall<OauthUser[]>('/views/resources/', 'GET'), [wrappedApiCall]);
    const get_zoho_events = useCallback((targetDate: string) => wrappedApiCall<any[]>(`/views/get_zoho_events?calendar_uid=b3be700c24914243ab0190bf9bddd75c&start_date=${targetDate}&end_date=${targetDate}`, 'POST'), [wrappedApiCall]);
    const zoho_oauth_initiate = useCallback(() => wrappedApiCall<any>(`/views/zoho_oauth_initiate`, 'GET'), [wrappedApiCall]);

    const exportDPPdf = useCallback((dpTestaId: number) => wrappedApiCall<{ message: string }>(`/views/export_dp_pdf/${dpTestaId}`, 'POST'), [wrappedApiCall]);
    ...
    const getTopResourcesReport = useCallback(() => wrappedApiCall<{ message: string; data: any[] }>('/views/reports/top_resources/', 'GET'), [wrappedApiCall]);
    const getInterventionsByPeriodReport = useCallback((startDate: string, endDate: string, clientName?: string, resourceAssigned?: string, interventionType?: string) => {
        ...
        return wrappedApiCall('/views/reports/interventions_by_period/', 'GET', null, queryParams);
    }, [wrappedApiCall]);
    ...
    const getClientsWithSedi = useCallback(() =>
        wrappedApiCall<any[]>('/clients/', 'GET'),
        [wrappedApiCall]
    );

    const createClient = useCallback((data: { ragione_sociale: string; sedi: { descrizione: string }[] }) =>
        wrappedApiCall<any>('/clients/', 'POST', data),
        [wrappedApiCall]
    );

    const updateClient = useCallback((id: number, data: { ragione_sociale: string }) =>
        wrappedApiCall<any>(`/clients/${id}`, 'PUT', data),
        [wrappedApiCall]
    );

    const deleteClient = useCallback((id: number) =>
        wrappedApiCall(`/clients/${id}`, 'DELETE'),
        [wrappedApiCall]
    );

    const createSede = useCallback((data: { id_cliente: number; descrizione: string }) =>
        wrappedApiCall<any>('/clients/sedi', 'POST', data),
        [wrappedApiCall]
    );

    const updateSede = useCallback((id: number, data: { descrizione: string }) =>
        wrappedApiCall<any>(`/clients/sedi/${id}`, 'PUT', data),
        [wrappedApiCall]
    );

    const deleteSede = useCallback((id: number) =>
        wrappedApiCall(`/clients/sedi/${id}`, 'DELETE'),
        [wrappedApiCall]
    );

    const getAllTipiIntervento = useCallback(() =>
        wrappedApiCall<TipoIntervento[]>('/interventions/', 'GET'),
        [wrappedApiCall]
    );

    const createTipoIntervento = useCallback((data: { descrizione: string }) =>
        wrappedApiCall<TipoIntervento>('/interventions/', 'POST', data),
        [wrappedApiCall]
    );

    const updateTipoIntervento = useCallback((id: number, data: { descrizione: string }) =>
        wrappedApiCall<TipoIntervento>(`/interventions/${id}`, 'PUT', data),
        [wrappedApiCall]
    );

    const deleteTipoIntervento = useCallback((id: number) =>
        wrappedApiCall(`/interventions/${id}`, 'DELETE'),
        [wrappedApiCall]
    );

    const getTop10Clienti = useCallback(() =>
        wrappedApiCall<any[]>(`/report_and_statistics/get_top_10_clienti`, 'GET'),
        [wrappedApiCall]
    );


    const getTop10Risorse = useCallback(() =>
        wrappedApiCall<any[]>(`/report_and_statistics/get_top_10_risorse`, 'GET'),
        [wrappedApiCall]
    );
    ...
    return {
        ...
        createTipoIntervento,
        updateTipoIntervento,
        deleteTipoIntervento,
        getTop10Clienti,
        getTop10Risorse
